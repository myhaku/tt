...
另可参见: [Colors](#colors), [Mathematics](#mathematics) and [Regular Expressions](#regular_expressions).
...
> 语法:#log {APPEND|OVERWRITE} {filename}

Log 命令将滚动区域中接收到的所有数据记录到指定的文件名中。模式为 o (覆盖) 或 a (附加)。
...
> 语法: #regex {string} {regex} {true} {false}
...
\%s |将零与任意数量的空格匹配|	([\r\n\t ]*?)
\%S	 |将零与任意数量的非空格匹配|	([^\r\n\t ]*?)
\%w |	将零与任意数量的单词字符匹配|([A-Za-z0-9_]*?)
\%W |	将零匹配到任意数量的非单词字符|([^A-Za-z0-9_]*?)
\%?	|匹配 0 个或 1 个字符|(.??)
\%.	|匹配一个字符|	(.)
\%+	|将一个字符与任意数量的字符匹配|(.+?)
\%*	|将零与任意数量的字符匹配|(.*?)
